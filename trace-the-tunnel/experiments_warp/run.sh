#!/bin/bash
# Harness for the trace-the-tunnel warp experiment.
#
# Clones experiments_replay/run.sh but reads prompts from
# experiments_warp/prompts/ and the solver from
# experiments_warp/warp_solver.js. The replay harness is left untouched.
#
# The experiment name determines:
#   - the prompt file:      experiments_warp/prompts/<NAME>.txt
#   - the output folder:    data/<NAME>/
#   - the server mode:      python server.py --experiment <NAME>
#
# Usage:
#   ./experiments_warp/run.sh warp_v1                    # 1 round
#   ./experiments_warp/run.sh warp_v1 --rounds 3         # 3 rounds
#
# Prereq: server must already be running with `--experiment <NAME>` and
# the v1b allowlist:
#   source .venv/bin/activate && python server.py --experiment warp_v1 \
...
PROMPT_FILE="experiments_warp/prompts/${NAME}.txt"
SOLVER_FILE="experiments_warp/warp_solver.js"
OUT_DIR="data/${NAME}"
LOG_DIR="data/logs"
...
if [[ -z "$SERVER_MODE" ]]; then
  ...
  echo "    --allowed-sessions experiments_replay/allowed_sessions_v1b.json"
  exit 1
fi
...
if [[ "$ACTUAL" != "$NAME" ]]; then
  ...
  echo "Restart the server with: python server.py --experiment $NAME --allowed-sessions experiments_replay/allowed_sessions_v1b.json"
  exit 1
fi
